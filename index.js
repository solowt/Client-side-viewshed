...
* @param {resolution} number - width/height of pixel in degrees, determines resolution of viewshed
...
function doClientVS(options){
  return new Promise((fulfill, reject)=>{
    let point = options.inputGeometry,
        radius = options.radius || 5000,
        resolution = options.pixelWidth || 100,
...
function buildBounds(circle, resolution){
  ...
  return new Promise((resolve,reject)=>{
    Promise.all(lineArray.map((line)=>{
      return geoEngineAsync.geodesicDensify(line.line, resolution, 9102).then((result)=>{
        return {
          name: line.name,
          line: result
        }
...
function geoPointToElevation(point, view){
  let hash = this.hashCode(point.longitude,point.latitude);
  if (this.elevationCache.has(hash)){
    return this.elevationCache.get(hash);
  } else {
    let height = view.basemapTerrain.getElevation(point);
    this.elevationCache.set(hash,height);
    return height;
  }
}

function pointToElevation(point,raster){
  let geoPoint = this.pointToGeoPoint(point,raster);
  return this.geoPointToElevation(geoPoint,raster.view);
}
...
function testLine(line,raster){
  ...
  return line.map((p)=>{
    if (p[0] === origin[0] && p[1] === origin[1]){
      ...
      let slope = this.slope(origin, p, raster);
